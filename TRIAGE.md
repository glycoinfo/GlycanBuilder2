...
Run through today's reader:

| | count |
|---|---|
| read back identically | **0** |
| read, but written back differently | 12 |
| **cannot be read at all** | **2** |

The 12 are benign and worth knowing: the written form appends the mass options the file omits, so
`…$MONO,Und,Na,0` comes back as `…$MONO,Und,Na,0,freeEnd`, and a file with no `$` at all gains
`$MONO,Und,0,0,freeEnd`. Both are the writer being more explicit than the writer of 2008 - not
damage. **It also settles the ion-cloud spelling: `Na` is what GlycoWorkbench wrote**, so the `Na1`
that came back as `0` earlier was a guess at the syntax, not a fault.

The 2 that fail are the finding:

```
dermatan_sulfate.gws  Invalid string format: <0;freeEnd--?GlcNAc(--4a?IdoA(…
nglycan_2605.gws      Invalid string format: <0;freeEnd--?b1D-GlcNAc>
```

Both begin `0;` - a leading field today's parser does not know, and a `;` which
`GlycanDocument.parseString` treats as the separator between structures. Whatever `0;` meant in 2008,
**GlycanBuilder2 cannot open two of the fourteen example files shipped with GlycoWorkbench.**

So the specification does have evidence to rest on, and the first thing that evidence says is that
the current reader is not a superset of the old writer. These 14 files belong in the repository as
test data before anything is rewritten - they cost nothing to add and they are the only witness we
have.
